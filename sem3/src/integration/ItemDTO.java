package integration;

/**
 * A DTO that contains information about an item that can be bought at the store.
 */
public final class ItemDTO {
	private final int price;
	private final String descript;
	private final double tax;
        private final String itemID;
        private final int quantity;
	
	/**
	 * Creates a new itemDTO containing information about an item in the store.
	 * @param price The price of the item.
	 * @param descript The description of the item.
	 * @param tax The tax percentage of the item,
	 * @param itemID The identifier of the item.
         * @param quantity quantity of the item
	 */
	public ItemDTO(int price, String descript, double tax, String itemID) {
            this.price      = price;
            this.descript   = descript;
            this.tax        = tax;
            this.itemID     = itemID;
            this.quantity   = 1;
	}
        
        /**
         * copies the value of another itemDTO
         * @param item, ItemDTO with values
         */
        
        public ItemDTO(ItemDTO item) {
            this.price      = item.price;
            this.descript   = item.descript;
            this.tax        = item.tax;
            this.itemID     = item.itemID;
            this.quantity   = item.quantity;
	}
        
        /**
         * copies the calue of another itemDTO, but changes the quantity.
         * @param item ItemDTO with values
         * @param quantity the new item quantity.
         */
        public ItemDTO(ItemDTO item, int quantity) {
            this.price      = item.price;
            this.descript   = item.descript;
            this.tax        = item.tax;
            this.itemID     = item.itemID;
            this.quantity   = quantity;
	}

	/**
	 * Gets the price of the item.
	 */
	public int getPrice() {
            return price;
	}
	
	/**
	 * Gets the description of the item.
	 */
	public String getDescription() {
            return descript;
	}
	
	/**
	 * Gets the tax percentage of the item.
	 */
	public double getTax() {
            return tax;
	}
	
        /**
         * Get ID for the item
         * @return itemID
         */
        public String getID() {
            return itemID;
        }
        
        /**
         * get the new quanity
         */
        public int getQuantity() {
            return quantity;
        }
        
        
        
        @Override
        public boolean equals(Object other) {
            if (other==null || !(other instanceof ItemDTO)){
                return false;
                   
               }
            ItemDTO otherItem= (ItemDTO) other;
            return this.getID().equals(otherItem.getID());
        }
}
